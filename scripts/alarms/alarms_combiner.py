...
INPUT_DIR = os.path.abspath(os.path.join(BASE_DIR, "..", "..", "datasets", "alarms", "alert_csvs"))
OUTPUT_FILE = os.path.abspath(os.path.join(BASE_DIR, "..", "..", "datasets", "alarms", "alarms_data.csv"))
...
def main():
    ...
    print(f"[i] Pass 1 dedup (raion->oblast): {before:,} -> {len(combined):,} rows")

    before  = len(combined)
    combined = merge_overlapping(combined).reset_index(drop=True)
    print(f"[i] Pass 2 dedup (overlap merge): {before:,} -> {len(combined):,} rows")
    ...
    print(f"[✓] Open-ended alarms (no alarm_end): {remaining_nat:,}")
