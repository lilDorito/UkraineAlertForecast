# [Manual] script to generate a confusion matrix of saved LinReg model
...
preds = np.column_stack([
    (m.predict(X_ho_scaled) > 0.5).astype(int)
    for m in estimators
])
...
ax.set_title("LinearRegression confusion matrix (all targets, holdout)")
plt.tight_layout()
plt.savefig(os.path.join(PLOTS, "confusion_linear.png"), dpi=150)
print("[+] Saved -> plots/confusion_linear.png")