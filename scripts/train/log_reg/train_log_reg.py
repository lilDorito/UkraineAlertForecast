# [Manual] script to train a Logreg model
...
for i, col in enumerate(target_cols):
    ...
    for tr, te in tscv.split(X_scaled):
        model = LogisticRegression(solver='saga', max_iter=200, tol=1e-2, n_jobs=-1)
...
print(f"\n[+] LogisticRegression  F1={np.mean(all_f1):.3f} | Prec={np.mean(all_prec):.3f} | Rec={np.mean(all_rec):.3f}")

estimators = []
for col in target_cols:
    m = LogisticRegression(solver='saga', max_iter=200, tol=1e-2, n_jobs=-1)
    m.fit(X_scaled, y[col])
    estimators.append(m)

joblib.dump(estimators, os.path.join(MODELS, "logistic_regression.pkl"))
joblib.dump(scaler, os.path.join(MODELS, "logistic_regression_scaler.pkl"))
